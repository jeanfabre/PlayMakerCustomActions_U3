...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip("Sends events when an object is touched. Optionally filter by a fingerID. NOTE: Uses the MainCamera! Only raycast on the gameObject layer, prevents mix up in some cases.")]
	public class TouchObjectEvent2 : FsmStateAction
	{
		[RequiredField]
			...
			if (Input.touchCount > 0)
			{
				...
				foreach (var touch in Input.touches)
				{
					if (fingerId.IsNone || touch.fingerId == fingerId.Value)
					{
						...
						RaycastHit hitInfo;
						Physics.Raycast(Camera.main.ScreenPointToRay(screenPos), out hitInfo, pickDistance.Value,1<<go.layer);
