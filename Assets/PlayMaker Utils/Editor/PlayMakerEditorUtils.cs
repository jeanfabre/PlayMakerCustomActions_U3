...
public class PlayMakerEditorUtils : Editor {
	...
	public static void ExportCurrentScene()
	{
		...
		if (EditorUtility.DisplayDialog("Export as package?","You can export a package, or just list the files","Export","Just list the selection"))
		{
			...
			foreach(var _object in Selection.objects)
			{
				...
				Debug.Log( AssetDatabase.GetAssetPath(_object));
			}
		}
